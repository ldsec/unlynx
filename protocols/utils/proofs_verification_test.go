...
func TestProofsVerification(t *testing.T) {
	...
	PublishedShufflingProof1 := libunlynxshuffle.ShuffleProofCreation(cipherVectorToShuffle, detResponsesCreationShuffled, libunlynx.SuiTe.Point().Base(), protocol.Roster().Aggregate, beta, pi)

	PublishedShufflingProof2 := libunlynxshuffle.ShuffleProofCreation(cipherVectorToShuffle, cipherVectorToShuffle, libunlynx.SuiTe.Point().Base(), pubKey, beta, pi)
